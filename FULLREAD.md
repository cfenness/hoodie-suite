...
- Hard rules: (a) **raw rows never leave the browser** — **ONLY for SipSource / anonymized industry data (N≥10), NOT a Hoodie rule.** Hoodie's pulls are PUBLIC scraped data, so the actual rows DO go to the model (decided 2026-06-29). Apply rows-never-leave per data source, not universally; (b) **the model never produces a displayed number** — it picks tiles/aggregations, the app computes every value; (c) **the deterministic layer works model-off**.
...
4. **Joins + multi-dataset.** vs a catalog (value-set / bloom-filter index) → the joinable fields that widen the scoped RB. **Load multiple RELATED datasets at once** (user, 2026-06-29: "use all of the fl files at once") — multi-file ingest in Overlay: add files to a working set, auto-detect shared keys, and let a tile draw from any/all of them (the scoped RB spans the combined field set). **Grain guardrail is ADVISORY/OVERRIDABLE, not a stop** (user, 2026-06-29): never block transaction/account-grain uploads; surface an N≥10 roll-up suggestion for externally shared output with a per-upload override toggle. (analyze.py prompt already softened to flag-not-refuse.)
