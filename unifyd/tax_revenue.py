...
Two authoritative, free sources — no scrape:
  • CENSUS STC (state) — the Annual Survey of State Government Tax Collections via the governments
    ...
    NOTE the govs `AMOUNT` field is historically reported in THOUSANDS of dollars; we store it raw with
    unit="USD_thousands" and DO NOT silently rescale — confirm the scale on the first live run, then flip
    _AMOUNT_SCALE if needed. Getting this wrong is a 1000x margin error, so it stays explicit.
...
_AMOUNT_SCALE = 1                     # keep raw (unit=USD_thousands); flip to 1000 only after live confirms scale
