...
SOURCES = [
    ...
    dict(id="ttb-enrich", label="TTB COLA enrich (detail+UPC)", code="import ttb_pull as m; m.run_enrich()",
         tables=["ttb_cola"], klass="headless", cadence="daily", enabled=True, timeout=7200,
         note="$0 off-Mac: deepens un-enriched ttb_cola rows (empty UPC) w/ detail fields + label-barcode UPC, concurrent, TTB_ENRICH_LIMIT/run; covers new + backfill"),
