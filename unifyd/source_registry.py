...
SOURCES = [
    ...
    dict(id="kroger", label="Kroger", code="import kroger_api as m; m.run()",
         tables=["kroger_products"], klass="creds", cadence="daily", enabled=True, note="Kroger API creds"),
    ...
    dict(id="stop-and-shop", label="Stop & Shop", code="import stop_and_shop as m; m.run()",
         tables=["stop_and_shop_products"], klass="headless", cadence="daily", enabled=True, note="never run yet — validate"),
    ...
    dict(id="census", label="US Census ACS", code="import census as m; m.run()" ,
         tables=["census_reference"], klass="creds", cadence="weekly", enabled=True, note="Census API key"),
