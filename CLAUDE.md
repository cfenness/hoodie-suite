...
- **`unifyd/xsource_queue.py` + `apps/match-trainer.html` — the continuous queue.** The CSV caps the
  ...
  **`unifyd/product_taxonomy.py` — the canonical Type → Class → Sub Class → Varietal hierarchy**
  ...
  a brewer's distinction and the shelf is organised the buyer's way. **Labels lead with the familiar
  term**: `Tequila / Agave Spirits`, not `Agave Spirits` — the precise term stays in the label, it is
  just not the part doing the finding. The first cut shipped 34 classes and was *way* short; the
  test now enforces a floor (≥70 classes) and refuses a term that sits at two different levels,
  which caught three classes nesting a sub class of their own name. Held as four free-text boxes the
  ...
  **whole path**: a Sub Class landed without its parents cannot be filtered under any Class, so it
  ...
  The seed is **served at constant speed** (~1ms): reading the learned-paths table before it has ever
  been written costs a DuckDB connection plus a slow-failing S3 round trip, measured live at
  **11–14 SECONDS** for a payload that is 99% a literal. Since the surface renders dropdowns off that
  response, twelve seconds of text boxes is indistinguishable from the feature not shipping — which is
  exactly how it was reported. The warehouse read never blocks: a miss serves the full seed and
  refreshes in the background, `learn()` invalidates the cache, and `learned_read` separates "nothing
  has been learned" from "we have not looked yet".
